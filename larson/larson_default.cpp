...
__attribute__((noinline)) void* malloc_1(size_t bytes)
{
  void *victim = malloc(bytes);
  return victim;
}

__attribute__((noinline)) void free_1(void* mem)
{
  free(mem);
}
...
#define CUSTOM_MALLOC malloc_1
#define CUSTOM_FREE   free_1
#endif
...
void runthreads(double sleep_cnt, int min_threads, int max_threads,
		int chperthread, int num_rounds) ;
void runloops(double sleep_cnt, int num_chunks ) ;
...
{
  // setAffinity(0);
  setMaxAffinity(NUM_MAX_THREADS);
...
void runthreads(double sleep_cnt, int min_threads, int max_threads, int chperthread, int num_rounds)
{
  ...
  for(num_threads=min_threads; num_threads <= max_threads; num_threads++ ) {
      ...
      for(i=0; i< num_threads; i++){
          ...
          de_area[i].desired_core = i;
          // de_area[i].desired_core = i+1;
      ...
      // Sleep(sleep_cnt * 2000) ; // wait 5 sec for old threads to die
...
static void * exercise_heap( void *pinput)
{
  ...
  for( cblks=0; cblks<pdea->NumBlocks; cblks++){
    // std::cout << "cblks: " << cblks << std::endl;
    victim = lran2(&pdea->rgen)%pdea->asize ;
    // victim = cblks % pdea->asize;
